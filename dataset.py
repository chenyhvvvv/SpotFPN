...
class VisiumHDDataset(data.Dataset):
    """
    Dataset class for Visium data.
    Input:
    ...
    def __getitem__(self, index):
        ...
        expr_aug_norm = np.log1p(expr_aug / (expr_aug.sum(axis=-1, keepdims=True) + 1e-6) * self.opt.normalized_total)

        expr_torch = torch.from_numpy(expr_aug).permute(2, 0, 1).float()
        expr_norm_torch = torch.from_numpy(expr_aug_norm).permute(2, 0, 1).float()
        nucl_torch = torch.from_numpy(nucl_aug).long()
        mask_torch = torch.from_numpy(mask).long()
        bg_mask_torch = 1 # torch.from_numpy(bg_mask).long()

        return expr_torch, expr_norm_torch, nucl_torch, mask_torch, bg_mask_torch, coords_h1, coords_w1
